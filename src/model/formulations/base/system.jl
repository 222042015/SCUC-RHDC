...
function _add_system_wide_eqs!(
    ...
    _add_net_injection_eqs!(model, sc, nCont, nInt)
    _add_spinning_reserve_eqs!(model, sc, nCont, nInt)
    _add_flexiramp_reserve_eqs!(model, sc, nCont, nInt)
    return
end
...
function _add_net_injection_eqs!(
    ...
    for t in 1:T, b in sc.buses
        n = net_injection[sc.name, b.name, t] = @variable(model, base_name = "net_injection_$(b.name)_$(t)" )
        # n = p
        eq_net_injection[sc.name, b.name, t] = @constraint(
            model,
            -n + model[:expr_net_injection][sc.name, b.name, t] == 0,
            base_name = "eq_net_injection_$(b.name)_$(t)"
        )
    end
    for t in 1:T
