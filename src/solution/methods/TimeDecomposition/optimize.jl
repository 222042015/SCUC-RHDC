...
instance = ucRH.read("instance.json")

solution = ucRH.optimize!(
    instance,
    TimeDecomposition(
...
function optimize!(
    ...
    for t_start in 1:method.time_increment:T
        ...
        sub_instance = ucRH.slice(instance, t_start:t_end)
        # build and optimize the model 
        sub_model = ucRH.build_model(
        ...
        ucRH.optimize!(sub_model, method.inner_method)
        # get the result of each time period
        sub_solution = ucRH.solution(sub_model)
